...
function setupJoystickEvents() {
    ...
    function handleEnd() {
        ...
        knob.style.background = 'rgba(255, 255, 255, 0.15)';
    }
...
function setupJumpEvents() {
    ...
    function jump() {
        if (player.jumps < maxJumps) {
            player.vy = -jumpPower;
            player.jumps++;
            jumpButton.style.transform = 'scale(0.85)';
            jumpButton.style.background = 'rgba(255, 255, 255, 0.15)';
            jumpButton.style.borderColor = 'rgba(255, 255, 255, 0.4)';
            setTimeout(() => {
                jumpButton.style.transform = 'scale(1)';
                jumpButton.style.background = 'rgba(0, 0, 0, 0.5)';
                jumpButton.style.borderColor = 'rgba(255, 255, 255, 0.15)';
            }, 100);
        }
    }
...
function crossFadeTo(src, duration=1200){
  ...
  inactiveAudio.play()
  inactiveAudio.playbackRate = 0.98 + Math.random() * 0.04
...
function applyTheme(i){
  ...
  if(gameMode === "story"){
    ...
    activeAudio.volume = 1
    activeAudio.playbackRate = 0.98 + Math.random() * 0.04
...
const z=-q.z
const s=FOV/z
const sx = q.x*s + rc.width/2
const sy = q.y*s + rc.height/2
const warp = 1 + Math.min(2, (z / 600) | 0)
const snap = PS1_SNAP * (1 + Math.min(1, warp * 0.5))

return {
  x: Math.round(sx / snap) * snap,
  y: Math.round(sy / snap) * snap,
  z
}
}
...
const c = themes[currentTheme].platform().match(/\d+/g).map(Number)
p.color = posterizeColor(c)
platformCount++
...
if(isMobile && joystick.active){
    ...
    const mobileSpeed = moveSpeed * 1.5;
    player.vx += (dx * forward + dz * strafe) * mobileSpeed;
    player.vz += (-dz * forward + dx * strafe) * mobileSpeed;
}
...
faceBuffer.push({
  pts:[pts[2],pts[3],pts[0]],
  depth:(pts[2].z+pts[3].z+pts[0].z)/3,
  color:p.color
})
  }
}
...
const wobble = Math.sin(performance.now() * 0.002 + foot.z * 0.01) * 2
...
x.drawImage(rc, frameJitterX, frameJitterY, W, H)
x.globalAlpha=0.6
x.drawImage(rc, frameJitterX, frameJitterY, W, H)
x.globalAlpha=1
for(let y=0;y<H;y+=2){
...
function loop(t){
  ...
  frameJitterX = (Math.random() - 0.5) * 0.6
  frameJitterY = (Math.random() - 0.5) * 0.6
...
const band = Math.floor(fog * 6) / 6
rx.globalAlpha = 1

rx.fillStyle = f.color
rx.save()
rx.globalCompositeOperation = "multiply"
rx.fillStyle = `rgba(80,80,80,${band})`
